...
const Header: React.FC<HeaderProps> = ({ logoUrl, currentUser, onNavigate, onLogout, onOpenSidebar }) => {
  return (
    <header className="bg-white/80 backdrop-blur-lg shadow-sm sticky top-0 z-10 border-b border-slate-200">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4 flex justify-between items-center">
        <div className="flex items-center space-x-3">
           <button onClick={onOpenSidebar} className="p-2 rounded-md text-slate-600 hover:bg-slate-100 lg:hidden">
           ...
           <button onClick={() => onNavigate('home')} className="flex items-center space-x-3">
              ...
              <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Trial Class Booker</h1>
          </button>
        </div>
        <div>
          {currentUser ? (
            <div className="flex items-center space-x-4">
              <span className="text-sm font-medium text-slate-700 hidden sm:block">Welcome, {currentUser.name.split(' ')[0]}!</span>
