...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, currentUser, onNavigate, onLogout }) => {
    ...
    <>
      ...
      >
          ...
          {currentUser ? (
            <div className="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-500">{currentUser.isAdmin ? 'Admin' : 'Student'}</p>
                </div>
            </div>
          ) : (
            <div className="flex items-center space-x-3">
        ...
        <nav className="flex-grow p-4 space-y-2">
            ...
            <hr/>
            {navItems.map(item => (
                (!item.id.includes('admin') || (currentUser?.isAdmin)) && (
                    <NavItem 
                        key={item.id}
                        icon={item.icon}
                        label={item.label}
